function romanToDecimal({ splittedNumber, romanLetters }) {
  if (
    splittedNumber.filter((letter) => !romanLetters.includes(letter)).length !==
    0
  ) {
    const myError = {
      code: 3,
  ...
  for (let i = 0; i < splittedNumber.length; i++) {
    ...
    const isLowerThanNext =
      charIndexAtRomanLetters < nextCharIndexAtRomanLetters;
    const decimalNumberToPush = isLowerThanNext
      ? decimalNumber * -1
      : decimalNumber;
...
function decimalToRoman({ splittedNumber, romanLetters }) {
  ...
  for (let i = 0; i < splittedNumber.length; i++) {
    ...
    if (digitAsInt % 5 && digitAsInt % 5 <= 3) {
      ...
      romanNumber +=
        romanLetters[
          digitAsInt < 5 ? secondRomanIndexByPos : thirdRomanIndexByPos
        ];
